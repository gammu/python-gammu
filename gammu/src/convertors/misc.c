...
GSM_CategoryType StringToCategoryType(const char *s)
{
	...
	else if (strcmp(s, "Phonebook") == 0)
		return Category_Phonebook;
	else {
		PyErr_Format(PyExc_ValueError,
			     "Bad value for category type: '%s'", s);
		return 0;
	}
}
...
char *MemoryTypeToString(GSM_MemoryType t)
{
	...
	if (strcmp("XX", s) == 0) {
		PyErr_Format(PyExc_ValueError,
			     "Bad value for MemoryType from Gammu: '%02x'", t);
		free(s);
		s = NULL;
	}
	return s;
...
PyObject *CBToPython(GSM_CBMessage * cb)
{
	return Py_BuildValue("{s:i,s:s}",
			     "Channel", cb->Channel, "Text", cb->Text);
}
...
GSM_USSDStatus StringToUSSDStatus(const char *s)
{
	if (strcmp(s, "Unknown") == 0)
		return USSD_Unknown;
	else if (strcmp(s, "NoActionNeeded") == 0)
		return USSD_NoActionNeeded;
	else if (strcmp(s, "ActionNeeded") == 0)
		return USSD_ActionNeeded;
	else if (strcmp(s, "Terminated") == 0)
		return USSD_Terminated;
	else if (strcmp(s, "AnotherClient") == 0)
		return USSD_AnotherClient;
	else if (strcmp(s, "NotSupported") == 0)
		return USSD_NotSupported;
...
char *USSDStatusToString(GSM_USSDStatus type)
{
	...
	if (s == NULL) {
		PyErr_Format(PyExc_ValueError,
			     "Bad value for USSD Status from Gammu: '%d'",
			     type);
		return NULL;
	}
...
PyObject *USSDToPython(GSM_USSDMessage * ussd)
{
	...
	result = Py_BuildValue("{s:s,s:O}", "Status", status, "Text", text);
